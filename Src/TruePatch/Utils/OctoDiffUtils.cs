...
namespace TruePatch.Utils
{
    public class OctoDiffUtils
    {
                ...
                FilePatchProgress progressReporter = null)
            {
                ...
                for (int i = 0; i < validChunkSizes; i++)
                {
                    ...
                    else
                    {
                        ...
                        {
                            TruePatchUtils.MoveFile(deltaPathTemp, deltaPath);
            ...
            private static void CalculateDeltaInternal(string sourcePath, string targetPath, string deltaPath,
                string signaturePath, int chunkSize, FilePatchProgress progressReporter = null)
            {
                using (var signatureStream =
                    new FileStream(signaturePath, FileMode.Create, FileAccess.ReadWrite, FileShare.Read))
                {
